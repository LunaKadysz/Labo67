...
voltage = np.array([]) # Create an empty array.
...
for k in range(1, N_POINTS + 1):
	...
	SR830.query("OUTP? 3") # If I don't query at least four times, it may return crap from some buffer (which I don't know how to clear).
	SR830.query("OUTP? 3") # If I don't query at least four times, it may return crap from some buffer (which I don't know how to clear).
	SR830.query("OUTP? 3") # If I don't query at least four times, it may return crap from some buffer (which I don't know how to clear).
	SR830.query("OUTP? 3") # If I don't query at least four times, it may return crap from some buffer (which I don't know how to clear).
	voltage = np.append(voltage, float(SR830.query("OUTP? 3")))
	SR830.query("OUTP? 4") # If I don't query at least four times, it may return crap from some buffer (which I don't know how to clear).
	SR830.query("OUTP? 4") # If I don't query at least four times, it may return crap from some buffer (which I don't know how to clear).
	SR830.query("OUTP? 4") # If I don't query at least four times, it may return crap from some buffer (which I don't know how to clear).
	SR830.query("OUTP? 4") # If I don't query at least four times, it may return crap from some buffer (which I don't know how to clear).
	phase = np.append(phase, float(SR830.query("OUTP? 4")))
...
header_str = 'frequency (Hz)	Voltage (V) 	phase (deg)'
np.savetxt('data/'+file_name+'.txt', np.transpose([frequency, voltage, phase]), fmt='%1.7e', delimiter='\t', header=header_str, newline='\n', comments='# ')
