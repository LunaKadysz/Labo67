...
row = info_muestras[info_muestras['Nombre'] == '05G-ULF']
muestra = ms.Muestra('05G-ULF', row['Tipo'].iloc[0], row['Anillos'].iloc[0], row['Contactos'].iloc[0], row['Soldaduras'].iloc[0], row['Heater'].iloc[0])
    
muestra.set_resistencias_mediciones_nit()
    
plt.figure(figsize=(18,13))
plt.title(f'{muestra.nombre} Nitrogeno',fontsize = 20)
plt.xlabel('$R_i$',fontsize = 20)
plt.ylabel('R[$k\Omega$]',fontsize = 20)
plt.xticks(fontsize = 15)
plt.yticks(fontsize = 15)
for i in range(muestra.medicion_nit.R['i'].min(),muestra.medicion_nit.R['j'].max()+1):

    df_i = muestra.medicion_nit.R[(muestra.medicion_nit.R['i']==i) | (muestra.medicion_nit.R['j']==i)].copy()
    print(df_i)
    idx = (df_i['j'] == i)
